...
export function ThemeToggle({ className, isDark: isDarkFromProps, onToggle }: ThemeToggleProps) {
  const [internalDark, setInternalDark] = useState(true);
  const isControlled = typeof isDarkFromProps === 'boolean';
  const isDark = isControlled ? isDarkFromProps : internalDark;

  const nextTheme = useMemo(() => !isDark, [isDark]);

  const handleToggle = () => {
    if (!isControlled) {
      setInternalDark((current) => !current);
    }
    onToggle?.(nextTheme);
  };
